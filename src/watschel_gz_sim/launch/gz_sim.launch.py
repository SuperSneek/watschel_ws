...
def generate_launch_description():
    ...
    pkg_watschel_control = get_package_share_directory('watschel_control')

    pkg_watschel_gz_sim = get_package_share_directory('watschel_gz_sim')
    world = os.path.join(pkg_watschel_gz_sim, 'worlds', 'flat.sdf')
    ...
    controllers = IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    os.path.join(pkg_watschel_control, 'launch', 'sim_control.launch.py')
                )
            )
    ...
    bridge = Node(
                ...
                parameters=[{
                    'config_file': os.path.join(pkg_watschel_gz_sim, 'config','gz_bridge.yaml'),
                }],
                output='screen'
            )
